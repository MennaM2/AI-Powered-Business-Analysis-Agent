...
def _markdown_to_html(markdown_text: str) -> str:
    ...
    enough for headers, bullets, and paragraphs so the report is
    readable in a browser without adding a markdown library."""
...
def _build_business_markdown(
    ...
    if period_comparison and "error" not in period_comparison:
        ...
        if period_comparison.get("top_growth"):
            ...
            lines.append("")
            for entry in period_comparison["top_growth"][:3]:
                lines.append(
        ...
        if period_comparison.get("top_decline"):
            ...
            lines.append("")
            for entry in period_comparison["top_decline"][:3]:
                lines.append(
                    f"- {entry['group']}: {entry['percent_change']}%"
                )
        lines.append("")
...
def generate_business_report(
    ...
    markdown_report = _build_business_markdown(
        ...
        resolved_group, key_findings, recommendations
    )
    html_report = _markdown_to_html(markdown_report)
