...
def health():
    """Liveness/readiness check, including whether Ollama is reachable."""
...
async def upload(file: UploadFile = File(...), session_id: str = None):
    ...
    If `session_id` is given and exists, the dataset is ADDED to that
    session (a new table alongside whatever was already there) - it
    never replaces or clears existing datasets or chat history. If
    `session_id` is omitted, a brand new session is created.
    """
    ...
    if session_id:
        ...
        if session is None:
            raise HTTPException(
                status_code=404, detail="Unknown session_id."
            )
    else:
    ...
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Unable to read CSV: {exc}"
        ) from exc

    memory.add_dataset(session.session_id, table_name, file_path)

    if not session.title:
        # Placeholder title from the dataset name, shown in "Resume a
        # chat" until (and if) the first question gives it a more
        # specific title - see chat_endpoint below.
        memory.set_title(session.session_id, _title_from_text(table_name))
...
def chat_endpoint(request: ChatRequest):
    ...
    try:
        result = run_agent(
            request.message, session.datasets, history=recent_history
        )
    except Exception as exc:
